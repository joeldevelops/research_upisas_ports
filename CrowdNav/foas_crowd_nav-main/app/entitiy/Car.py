...
class Car:
    ...
    def setArrived(self, tick):
        ...
        if tick > Config.initialWaitTicks and self.smartCar:  # as we ignore the first 1000 ticks for this
            ...
            oldAverageOverhead = CarRegistry.totalTripOverheadAverage
            ...
            # CarRegistry.totalTripOverheadVariance = addToVariance(CarRegistry.totalTrips,
            #                                                     CarRegistry.totalTripOverheadVariance,
            #                                                     oldAverageOverhead,
            #                                                     CarRegistry.totalTripOverheadAverage,
            #                                                     tripOverhead)
            # log to kafka
            ...
            # Set the most recent value of trip overhead and tick for HTTP Server
            
            simulationData = SimulationData()
            simulationData.set_trip_overhead(tripOverhead)
            simulationData.set_average_trip_overhead(CarRegistry.totalTripOverheadAverage)
            simulationData.set_average_trip_duration(CarRegistry.totalTripAverage)
            # simulationData.set_variance_trip_overhead(CarRegistry.totalTripOverheadVariance)
            simulationData.set_tick_number(tick)  
