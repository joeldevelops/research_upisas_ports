...
class Adaptive_Planner(gym.Env):
    def __init__(self,exploit,q_table):
        ...
        self.discretization_intervals_state = {
            'tripOverhead': (1, 3, self.binning_state),
        }
    ...
    def step(self, monitored_values,step_number, q_table_name):
        print("Another step")
        self.q_table_filename = q_table_name
    ...
    def calculate_reward(self, state):
        ...
        clipped_overhead = None
        if (state>min_overhead):
            if(state<max_overhead):
                clipped_overhead = state
            else:
                clipped_overhead = max_overhead
        else:
            clipped_overhead = min_overhead
