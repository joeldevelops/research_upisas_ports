...
class Simulation(object):
    ...
    @classmethod
    def httpMonitor(cls):
        """ writes config to a file for monitoring and executing """
        # Set the simulation data fields for HTTP Server
        try:
            simulationData = SimulationData()
            # Set the most recent value of exploration percentage for HTTP Server
            simulationData.set_exploration_percentage(CustomRouter.explorationPercentage)
            # Set the most recent value of average edge duration factor for HTTP Server
            simulationData.set_average_edge_duration_factor(CustomRouter.averageEdgeDurationFactor)
            # Set the most recent value of max speed and length factor for HTTP Server
            simulationData.set_max_speed_and_length_factor(CustomRouter.maxSpeedAndLengthFactor)
            # Set the most recent value of freshness update factor for HTTP Server
            simulationData.set_freshness_update_factor(CustomRouter.freshnessUpdateFactor)
            # Set the most recent value of freshness CutOff value for HTTP Server
            simulationData.set_freshness_cutoff_value(CustomRouter.freshnessCutOffValue)
            # Set the most recent value of reRoute every ticks for HTTP Server
            simulationData.set_reroute_every_ticks(CustomRouter.reRouteEveryTicks)
            # Set the most recent value of route random sigma for HTTP Server
            simulationData.set_route_random_sigma(CustomRouter.routeRandomSigma)
    ...
    def loop(cls):
        ...
        while 1:
            ...
            # Set the most recent value of routing and tick duration for HTTP Server
            simulationData = SimulationData()
            simulationData.set_routing_duration(current_milli_time() - timeBeforeCarProcess)
            simulationData.set_tick_duration(duration)
            ...
            if (cls.tick % 10) == 0:
                # Check if PUT call has happened and the execute_data.json file exists
                if Config.kafkaUpdates is False and Config.mqttUpdates is False and os.path.exists('../execute_data.json') is False:
                    # json mode
                    cls.applyFileConfig()
                    # Write to file for monitoring
                    cls.httpMonitor()

                if Config.httpUpdates is True and os.path.exists('../execute_data.json') is True:
                    # read from PUT written file and re-set values of Custom Router here
                    config = json.load(open('../execute_data.json'))
                    #Commented out values not beign changed as part of managing system
                    # CustomRouter.explorationPercentage = config['explorationPercentage']
                    CustomRouter.averageEdgeDurationFactor = config['averageEdgeDurationFactor']
                    CustomRouter.maxSpeedAndLengthFactor = config['maxSpeedAndLengthFactor']
                    CustomRouter.freshnessUpdateFactor = config['freshnessUpdateFactor']
                    # CustomRouter.freshnessCutOffValue = config['freshnessCutOffValue']
                    # CustomRouter.reRouteEveryTicks = config['reRouteEveryTicks']
                    # CustomRouter.routeRandomSigma = config['routeRandomSigma']
